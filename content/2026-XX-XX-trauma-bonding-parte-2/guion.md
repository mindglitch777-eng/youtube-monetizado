Formato: short

<!--
Short standalone, parte 2 de 2 de "trauma bonding" (no es un video largo de 6 bloques: no se valida con
validar_guion.py). Lo procesa scripts/generar_short.py.

- Cada línea de texto es una línea narrada; la palabra en **negrita** es la
  que se resalta en amarillo en el subtítulo.
- "# Escena N" agrupa líneas bajo una misma imagen (con efecto Ken Burns).
  El comentario "imagen:" es la descripción de la escena para Cloudflare y
  "variante:" la variante de color de PROMPTS.md (media = sin variante extra;
  fria = Cuerpo; calida = Pago).
- "sonido: whoosh" al principio de una escena = whoosh en el corte (todas
  las escenas menos la primera).
  "sonido: ding" en una línea = ding en su palabra en negrita; en el resto
  de las líneas suena un click suave cuando aparece la palabra en negrita.
  El sting de cierre se agrega solo al final.
-->

# Escena 5
<!-- variante: fria -->
<!-- imagen: A figure standing between two doors: a warm glowing door they turn their back on, and a dark cold door they are drawn into, confused and sad. -->

That's why someone **GOOD** bores you.
Your brain calls it '**suspicious**'.
Toxic feels '**familiar**'.
Healthy feels '**wrong**'.
That's what the **hijack** looks like.

# Escena 6
<!-- variante: media -->
<!-- imagen: A figure standing tall and snapping a red thread tied around their wrist, first warm sunlight entering from one side, brave and hopeful. -->
<!-- sonido: whoosh -->

This doesn't happen because you're **weak**.
It happens to the **strongest** people you know.
Because it takes **strength** to survive chaos.
And that strength is your way **out**.

# Escena 7
<!-- variante: calida -->
<!-- imagen: A small figure walking toward a bright warm sunrise, a broken red thread left on the ground behind them, peaceful, free and relieved. -->
<!-- sonido: whoosh -->

Next time you can't stop **thinking** about them...
...say **this**:
It's not **love**.
It's your brain, **drugged**. <!-- sonido: ding -->
And a drug can be **quit**.
Comment 'THIS IS **REAL**' if you recognized yourself.
And **send** it to someone who needs to hear it.
